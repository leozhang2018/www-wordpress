  ...
  $.fn.immersive_slider = function(options){
    ...
    if (settings.cssBlur == true) {
      el.find(".is-slide img:first-child").each(function( index ) {
        ...
        $(settings.container).addClass("is-container").prepend("<div id='slide-" + (index + 1) + "-bg' class='is-slide-background gs-cssblur " + activeclass + "'>" + img.clone().wrap("<div />").parent().html() + "</div>")
        $("#slide-" + (index + 1) + "-bg").positionSlides(settings, index)
      });
    } else {
      el.find(".is-slide").each(function( index ) {
        ...
        $(settings.container).addClass("is-container").prepend("<div id='slide-" + (index + 1) + "-bg' class='is-slide-background " + activeclass + "'>" + img + "</div>")
        $("#slide-" + (index + 1) + "-bg").positionSlides(settings, index)
      });
    }
    ...
    $(settings.container).find(".is-slide-background").wrapAll( "<div class='is-bg-overflow' />");
    el.find(".is-slide").wrapAll( "<div class='is-overflow' />");
    el.find(".is-slide").each(function( index ) {
      var activeclass = ""
      if(index == 0) activeclass = "active"
      $(this).attr("id","slide-" + (index + 1)).addClass(activeclass)
      $(this).positionSlides(settings, index)
      if(settings.pagination == true) {
        pagination += "<li><a class='is-page " + activeclass + "' href='#slide-" + (index + 1) + "'></a></li>"
      }
    });

    $("<div class='pagination-content'><ul class='is-pagination'>"+pagination+"</ul></div>").appendTo('.slide-wrapper')
   ...
   $(".is-prev").click(function() {
    ...
    $.fn.moveSlider = function(settings, page_index) {
      var el = $(this),
          ...
          bg_current = $(settings.container).find(".is-slide-background.active"),
          bg_next = $(settings.container).find(".is-slide-background" + page_index + "-bg");
      if(next) {
      ...
      pos = ((page_index.replace('#slide-','') - 1) * 100) * -1;
    ...
    $.fn.moveNext = function() {
      var el = $(this),
        ...
        page_index_number = parseInt($(this).find(".is-slide.active").attr("id").replace('slide-','')) + 1;
      if(page_index_number < total) {
        el.moveSlider(settings, "#slide-" + page_index_number)
      } else {
        if (settings.loop == true ) el.moveSlider(settings, "#slide-1")
      }
    }
    ...
    $.fn.movePrev = function() {
      ...
      if(page_index_number <= total && page_index_number > 0) {
        ...
        if (settings.loop == true ) el.moveSlider(settings, "#slide-" + (total - 1 ))
      }
    }
